...
const Avatar = ({ src, name, size = 'md' }) => {
  ...
  if (src && !imgError) {
    return <img src={src} alt={name} referrerPolicy="no-referrer" onError={() => setImgError(true)}
      className={`${sizeClass} rounded-full object-cover border-2 border-pastel-pink flex-shrink-0`} />;
  }
  return <div className={`${sizeClass} rounded-full bg-chocolate text-white flex items-center justify-center font-bold border-2 border-pastel-pink flex-shrink-0`}>{initials}</div>;
};

const TestimoniCard = ({ testimonial }) => {
  // Tampilkan badge "Baru" jika testimoni dibuat dalam 3 hari terakhir
  const isNew = testimonial.createdAt
    ? (Date.now() - (testimonial.createdAt.toDate?.() ?? new Date(testimonial.createdAt)).getTime()) < 3 * 24 * 60 * 60 * 1000
    : false;
  ...
  <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}
    ...
    {isNew && <span className="absolute top-4 right-4 bg-green-500 text-white text-xs px-2 py-0.5 rounded-full font-medium">Baru</span>}
    <div className="text-pastel-pink mb-4"><FiMessageSquare className="w-8 h-8" /></div>
...
const Testimoni = () => {
  ...
  const { testimonials, loading, loadingMore, hasMore, loadMore, addTestimoni } = useTestimoni();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast('Gagal mengirim testimoni', 'error');
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <AnimatePresence>
          {submitted && (
            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
              ...
              ✅ Terima kasih! Testimoni kamu sudah ditambahkan.
            </motion.div>
          )}
        </AnimatePresence>

        {loading ? (
          <div className="mb-8"><TestimoniGridSkeleton count={4} /></div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-8">
            {testimonials.map((t) => <TestimoniCard key={t.id} testimonial={t} />)}
          </div>
        )}
        ...
        {!loading && hasMore && (
          <div className="text-center mb-8">
            <motion.button onClick={loadMore} disabled={loadingMore}
              ...
              className="bg-cream dark:bg-gray-700 border-2 border-chocolate/20 text-chocolate dark:text-pastel-pink px-8 py-3 rounded-full font-medium hover:border-chocolate transition-all inline-flex items-center space-x-2 disabled:opacity-60">
              {loadingMore
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center">
          {!user && !showForm && (
            <div className="flex flex-col items-center space-y-3">
              <p className="text-gray-500 dark:text-gray-400 text-sm">Login dengan Google untuk menulis testimoni</p>
              <motion.button onClick={handleGoogleLogin} disabled={authLoading}
                whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                className="bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-600 text-gray-700 dark:text-gray-300 px-8 py-3.5 rounded-full font-semibold hover:border-chocolate hover:text-chocolate transition-all inline-flex items-center space-x-3 shadow-md disabled:opacity-60">
                {authLoading
          ...
          {user && !showForm && (
            <div className="flex flex-col items-center space-y-4">
              <div className="flex items-center space-x-3 bg-cream dark:bg-gray-800 px-5 py-3 rounded-full shadow-sm">
                <Avatar src={user.photoURL} name={user.displayName} size="sm" />
                <span className="text-gray-700 dark:text-gray-300 font-medium text-sm">{user.displayName}</span>
                <button type="button" onClick={() => { authService.logout(); setShowForm(false); }}
